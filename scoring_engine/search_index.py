...
def index_corunning(session: Session, race_id: int) -> None:
    from database.models import Race, RaceCoRunning
    ...
    row = session.query(RaceCoRunning).filter_by(race_id=int(race_id)).first()
    if not row or not isinstance(row.items, dict) or not row.items:
        return
    lines = []
    for k, v0 in row.items.items():
        if not isinstance(v0, dict):
            continue
        hn = str(k)
        name = str(v0.get("horse_name") or v0.get("name") or "").strip()
        com = str(v0.get("commentary") or v0.get("comment") or "").strip()
        if not com:
            continue
        lines.append(f"[{hn}] {name} {com}".strip())
        if len(lines) >= 40:
            break
    txt = " ".join([rk, "corunning", " ".join(lines)])
    upsert_search_document(
        session,
        doc_type="corunning",
        ref_key=f"corunning:{rk}",
        entity_type="race",
        entity_key=rk,
        race_id=int(race.id),
        race_date_day=rd,
