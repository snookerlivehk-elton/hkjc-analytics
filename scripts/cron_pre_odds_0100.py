...
def main():
    ...
    try:
        ...
        if not _is_within_window(now_hk):
            print(f"outside window now_hk={now_hk.isoformat()}")
            return
        ...
        for rid, rn, v in races:
            ...
            done_key = f"pre_odds_0100_done:{date_str}:{rno}"
            ...
            exists = (
                ...
                .filter(OddsHistory.odds_type == "PRE_0100")
                .limit(1)
                .all()
            )
            if exists:
                _upsert_cfg(session, done_key, True, f"賽前賠率快照（01:00）完成（{date_str} R{rno}）")
                ok_races += 1
                continue
