...
def main():
    st.set_page_config(page_title="練馬師統計（SP）", layout="wide")
    require_superadmin("練馬師統計（SP）")
    render_admin_nav(active="trainer_stats")
    st.title("🏇 練馬師勝出/入圍統計（SP）")
    ...
    try:
        ...
        st.download_button(
            ...
            file_name=f"trainer_stats_sp_{d1.isoformat()}_{d2.isoformat()}.csv",
            mime="text/csv",
