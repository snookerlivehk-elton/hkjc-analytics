        ...
        if not available_dates:
            ...
            if not preds:
                ...
                with st.expander("🤖 權重建議（Top5 模型）", expanded=False):
                    ...
                    w_rows = (
                        session.query(ScoringWeight.factor_name, ScoringWeight.weight)
                        .filter(ScoringWeight.is_active == True)
                    ...
                    max_w = float(c1.selectbox("建議權重上限", [2.0, 3.0, 4.0, 5.0], index=1, key="hit_factor_tune_max_w"))
                    top_k = int(c2.selectbox("TopK 定義", [5], index=0, key="hit_factor_tune_topk"))
                    run = c3.button("生成建議", use_container_width=True, key="hit_factor_tune_run_btn")

                    sig = (d1.isoformat(), d2.isoformat(), float(max_w), int(top_k))
                    if run:
                        ...
                        st.session_state["hit_factor_tune_result"] = res
                        st.session_state["hit_factor_tune_sig"] = sig

                    res = st.session_state.get("hit_factor_tune_result")
                    if st.session_state.get("hit_factor_tune_sig") != sig:
                        res = None
                    ...
                    if isinstance(res, dict) and res.get("ok") is True:
                        ...
                        st.download_button(
                            ...
                            key="hit_factor_tune_download_btn",
                        )

                        st.markdown("---")
                        st.caption("套用會直接更新 ScoringWeight（全局權重）。套用後可選擇立即重算該日期範圍內的所有場次。")
                        apply_rescore = st.checkbox("同時重算所選日期範圍", value=False, key="hit_factor_tune_apply_rescore")
                        confirm = st.text_input("輸入 APPLY 以套用", value="", key="hit_factor_tune_apply_confirm")
                        if st.button("套用到全局權重", use_container_width=True, key="hit_factor_tune_apply_btn"):
                            if str(confirm or "").strip().upper() != "APPLY":
                                st.warning("請先輸入 APPLY 再套用。")
                            else:
                                for fn, w in (sugg or {}).items():
                                    try:
                                        ww = session.query(ScoringWeight).filter_by(factor_name=str(fn)).first()
                                        if ww:
