...
from database.models import EntryFact, Jockey
...
def main():
    st.set_page_config(page_title="騎師統計（SP）", layout="wide")
    require_superadmin("騎師統計（SP）")
    render_admin_nav(active="jockey_stats")
    st.title("🏇 騎師勝出/入圍統計（SP）")
    ...
    try:
        ...
        rows_j = (
            session.query(Jockey.id, Jockey.name_ch)
            .join(EntryFact, EntryFact.jockey_id == Jockey.id)
            .filter(EntryFact.race_date_day >= d1)
            .filter(EntryFact.race_date_day <= d2)
            .distinct()
            .order_by(Jockey.name_ch.asc())
            .all()
        )
        ...
        q = base
        if jockey_sel[0] != "ALL":
            q = q.filter(EntryFact.jockey_id == int(jockey_sel[0]))
        if venue != "ALL":
        ...
        name_map = {int(i): str(n or "").strip() or f"J{i}" for i, n in rows_j}
        df["jockey_name"] = df["jockey_id"].map(name_map).fillna(df["jockey_id"].astype(str))

        df["win_rate"] = (df["win_cnt"] / df["samples"]).fillna(0.0)
        df["place_rate"] = (df["place_cnt"] / df["samples"]).fillna(0.0)

        show_cols = ["jockey_name"] + [c for c in cols if c != "jockey_id"] + ["samples", "win_cnt", "place_cnt", "win_rate", "place_rate"]
        ...
        st.download_button(
            ...
            file_name=f"jockey_stats_sp_{d1.isoformat()}_{d2.isoformat()}.csv",
            mime="text/csv",
